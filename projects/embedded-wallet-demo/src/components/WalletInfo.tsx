...
import { NfdAvatar, useAccountInfo, useNfd } from '@txnlab/use-wallet-ui-react'
...
export function WalletInfo() {
  ...
  if (!activeAddress) {
    return (
      <div className="text-center p-8 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700">
        ...
        <p className="text-gray-600 dark:text-gray-300">Connect your Algorand wallet to view your NFD profile and balance</p>
      </div>
    )
  }
  ...
  if (nfdQuery.isLoading || accountQuery.isLoading) {
    return (
      <div className="text-center p-8 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700">
        <p className="text-gray-600 dark:text-gray-300">Loading wallet data...</p>
      </div>
    )
  }
    ...
    <div className="p-8 bg-white dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">{nfd?.name || formatShortAddress(activeAddress)}</h2>
            {nfd?.name && <p className="text-sm text-gray-500 dark:text-gray-400 font-mono">{formatShortAddress(activeAddress)}</p>}
          </div>
        </div>

        <div className="px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <p className="text-sm text-gray-500 dark:text-gray-400">Balance</p>
          <p className="text-xl font-bold text-gray-900 dark:text-white">{formatNumber(algoBalance, { fractionDigits: 4 })} ALGO</p>
        </div>
      </div>

      {nfd?.properties?.userDefined && Object.keys(nfd.properties.userDefined).length > 0 && (
        <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-700">
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-4">NFD Properties</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {Object.entries(nfd.properties.userDefined).map(([key, value]) => (
